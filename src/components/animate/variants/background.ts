...
export const varBgColor = (props: IBgColorProps) => {
...
interface IBgKenBurnsProps {
  duration: number;
  ease?: string
}

export const varBgKenburns = (props: IBgKenBurnsProps) => {
  const duration = props?.duration || 5;
...
interface IBgPanProps {
  colors: string[];
  duration: number;
  ease: string;
}

export const varBgPan = (props: IBgColorProps) => {
